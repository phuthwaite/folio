jQuery(document).ready(function($){
	...
	function initializeLoadingBar(section) {
		var	selectedItem =  dashboard.find('.selected'),
			...
			barWidth = selectedItem.outerWidth(),
			barTop = selectedItem.offset().top,
			windowHeight = $(window).height(),
			windowWidth = $(window).width(),
			maxOffset = ( barTop + barWidth/2 > barWidth/2 ) ? barTop : windowWidth- barTop - barWidth,
			scaleValue = ((2*maxOffset+barWidth)/barWidth).toFixed(3)/1 + 0.001;
